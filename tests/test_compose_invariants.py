...
case("1/26 compliant compose passes", GOOD, None)
case("2/26 published host port is rejected",
     GOOD.replace('    expose:\n      - "8000"',
                  '    ports:\n      - "8000:8000"', 1),
     "ports")
...
case("6/26 EMBEDDING_ENDPOINT is rejected",
     GOOD.replace("      - EMBEDDING_DIMENSIONS=1536",
                  ...
                  "      - EMBEDDING_DIMENSIONS=1536"),
     "EMBEDDING_ENDPOINT")
case("7/26 wildcard CORS is rejected",
     GOOD.replace("CORS_ALLOWED_ORIGINS=https://cognee.aakashe.org",
                  "CORS_ALLOWED_ORIGINS=*"),
     "CORS")
case("8/26 default JWT secret is rejected",
     GOOD.replace("FASTAPI_USERS_JWT_SECRET=${COGNEE_JWT_SECRET}",
                  "FASTAPI_USERS_JWT_SECRET=super_secret"),
     "super_secret")
case("9/26 renamed service is rejected",
     GOOD.replace("  cognee-backend:", "  cognee:"),
     "cognee-backend")
case("10/26 missing EMBEDDING_DIMENSIONS is rejected as irreversible",
     GOOD.replace("      - EMBEDDING_DIMENSIONS=1536\n", "", 1),
     ["EMBEDDING_DIMENSIONS", "irreversible"])
case("11/26 wrong EMBEDDING_DIMENSIONS value is rejected as irreversible",
     GOOD.replace("EMBEDDING_DIMENSIONS=1536", "EMBEDDING_DIMENSIONS=3072"),
     ["EMBEDDING_DIMENSIONS", "irreversible"])
case("12/26 missing GRAPH_DATABASE_PROVIDER is rejected",
     GOOD.replace("      - GRAPH_DATABASE_PROVIDER=kuzu\n", "", 1),
     "GRAPH_DATABASE_PROVIDER")
case("13/26 wrong GRAPH_DATABASE_PROVIDER value is rejected",
...
case("14/26 DB_HOST=postgres is rejected",
     GOOD.replace("DB_HOST=cognee-postgres", "DB_HOST=postgres"),
     ["DB_HOST", "cognee-postgres"])
case("15/26 missing DB_HOST is rejected",
     GOOD.replace("      - DB_HOST=cognee-postgres\n", "", 1),
     "DB_HOST")
case("16/26 VECTOR_DB_HOST=postgres is rejected",
     GOOD.replace("VECTOR_DB_HOST=cognee-postgres", "VECTOR_DB_HOST=postgres"),
     ["VECTOR_DB_HOST", "cognee-postgres"])
case("17/26 missing VECTOR_DB_PASSWORD is rejected",
...
case("18/26 a deleted FASTAPI_USERS_JWT_SECRET is rejected",
     GOOD.replace("      - FASTAPI_USERS_JWT_SECRET=${COGNEE_JWT_SECRET}\n", "", 1),
     ["FASTAPI_USERS_JWT_SECRET", "default"])
case("19/26 missing COGNEE_API_AUTH_SCHEME on cognee-mcp is rejected",
     GOOD.replace("      - COGNEE_API_AUTH_SCHEME=x-api-key\n", "", 1),
     ["COGNEE_API_AUTH_SCHEME", "expires"])
case("20/26 COGNEE_API_AUTH_SCHEME=bearer is rejected",
     GOOD.replace("COGNEE_API_AUTH_SCHEME=x-api-key",
                  "COGNEE_API_AUTH_SCHEME=bearer"),
     ["COGNEE_API_AUTH_SCHEME", "x-api-key"])
case("21/26 missing MCP_ALLOWED_HOSTS is rejected",
     GOOD.replace("      - MCP_ALLOWED_HOSTS=cognee-mcp:*,"
                  "cognee-mcp.aakashe.org:*\n", "", 1),
     ["MCP_ALLOWED_HOSTS", "loopback"])
# The 2026-09-23 regression: a `networks:` block was added to cognee-backend
# alone. That stops Coolify injecting its set into EVERY service in the stack,
# so the three services that named nothing silently fell off `coolify` --
# metamcp lost DNS to cognee-mcp, which reads as the MCP server being down.
case("22/26 a service with no networks: block at all is rejected",
     GOOD.replace("    networks:\n      - default\n      - coolify\n"
                  "    environment:\n      - API_URL", "    environment:\n"
                  "      - API_URL", 1),
     ["cognee-mcp", "declares no `networks:`"])
case("23/26 cognee-mcp off the coolify network is rejected",
     GOOD.replace("    networks:\n      - default\n      - coolify\n"
                  "    environment:\n      - API_URL",
                  "    networks:\n      - default\n"
                  "    environment:\n      - API_URL", 1),
     ["cognee-mcp", "coolify", "metamcp"])
# The inverse mistake: "fix" the above by putting everything on coolify,
# which exposes the database to every other Coolify application.
case("24/26 cognee-postgres ON the coolify network is rejected",
     GOOD.replace("  cognee-postgres:\n    image: pgvector/pgvector:pg17\n"
                  "    networks:\n      - default",
                  "  cognee-postgres:\n    image: pgvector/pgvector:pg17\n"
                  "    networks:\n      - default\n      - coolify"),
     ["cognee-postgres", "must NOT join"])
case("25/26 an MCP_ALLOWED_HOSTS entry without the ':*' suffix is rejected",
...
print("\n26/26 the real deploy/cognee-selfhost.compose.yaml is compliant")
